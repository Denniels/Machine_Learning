...
def train_test_over_params(model, params, X_train, X_test, y_train, y_test):
    ...
    for i in values:
        ...
        tmp_model = model.set_params(**params_spec).fit(X_train, y_train).astype(int)
        tmp_train.append(mean_squared_error(y_train, tmp_model.predict(X_train))).astype(int)
        tmp_test.append(mean_squared_error(y_test, tmp_model.predict(X_test))).astype(int)
